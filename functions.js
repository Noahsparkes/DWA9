...
import { books, genres, authors, BOOKS_PER_PAGE } from "./data";
    ...
    if (books[i].id === bookId) {
      ...
      bookSummary.innerHTML =
        ...
        `<div class="overlay__preview">
          ...
          <h3 class="overlay__title" data-list-title="">${
            books[i].title
          } (${new Date(books[i].published).getFullYear()})</h3>
  ...
  `<div class="overlay__content">
        <form class="overlay__form" data-search-form="" id="search">
          ...
          <label class="overlay__field">
            ...
            <select class="overlay__input overlay__input_select" data-search-genres="" name="genre">${genreArray.map(
              (genreArray) =>
                `<option value="${genreArray}">${genreArray}</option>`
            )}</select>
          </label>
    
          <label class="overlay__field">
            <div class="overlay__label">Author</div>
            <select class="overlay__input overlay__input_select" data-search-authors="" name="author">${authorArray.map(
              (authorArray) =>
                `<option value="${authorArray}">${authorArray}</option>`
            )}</select>
          </label>
        </form>
...
export const searchBooks = (event) => {
  ...
  if (selectedGenre !== "All Genres") {
    ...
    filteredBooks = filteredBooks.filter((book) =>
      book.genres.includes(genreId.toString())
    );
  }

  if (selectedAuthor !== "All Authors") {
    const authorId = Object.keys(authors).find(
      (key) => authors[key] === selectedAuthor
    );
    filteredBooks = filteredBooks.filter((book) =>
      book.author.includes(authorId)
    );
  }
  ...
  if (
    ...
    document.querySelector(
      "[data-list-items]"
    ).innerHTML = `<div class = "list__message list__message_show" data-list-message = "">
                                                                <p>No results found.
 ...
 */
export const lightToggleDialog = document.querySelector(
  "[data-settings-overlay]"
);

lightToggleDialog.innerHTML =
  /* html */
  `<div class="overlay__content">
